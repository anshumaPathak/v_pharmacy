...
class RegisterViewModel with ChangeNotifier {
  ...
  Future<void> registerApi(
    ...
    final data = {
      "name": name,
      "mobile": mobile,
      if (email.isNotEmpty) "email": email, // ✅ Email tabhi bhejo jab diya ho
    };
    ...
    try {
      ...
      if (error.toString().contains("Email already exists")) {
        ...
        Utils.show("Email already registered", context);
      } else {
        Utils.show("Something went wrong. Please try again later.", context);
      }
    } finally {
  ...
  //     print("Register API Error: $error");
  //
  //     // 🔹 Agar error me "Email already exists" likha ho, to wahi message dikhao
  //     if (error.toString().contains("Email already exists")) {
  //       Utils.show("Email has already been taken", context);
  //     } else if (error.toString().contains("Conflict")) {
  //       Utils.show("Email already registered", context);
