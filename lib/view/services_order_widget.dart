...
import 'package:v_pharmashing/utils/url_tracking_helper.dart';
...
class _OrderMedicineScreenState extends State<OrderMedicineScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<DisasesCategoryViewModel>(
        context,
        listen: false,
      ).diseaseCategoryApi();
      _refreshProfile();
  ...
  // Track Meta Pixel event with URL parameters
  void _trackMetaPixelEvent(
    String eventName, {
    Map<String, dynamic>? parameters,
  }) {
    if (kIsWeb) {
      // Use URL tracking helper to include UTM parameters automatically
      UrlTrackingHelper.trackMetaPixelEventWithParams(
        eventName,
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> categories =
        categoryVM.disaseCategoryModel?.data
            ?.map((e) => e.name ?? "")
            .toList() ??
        [];
    ...
    final durations = ["7 Days", "15 Days", "30 Days", "60 Days", "90 Days"];
                                        ...
                                        child: Container(
                                          ...
                                          child: Stack(
                                            ...
                                            children: [
                                              ...
                                              Positioned(
                                                left:
                                                    50, // 👈 apne input ke hisab se adjust karlo
                                                child:
  ...
  Widget _buildSubmitButton() {
    ...
    // Create hidden HTML button for Meta Pixel detection (web only)
    if (kIsWeb) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        try {
          // Remove existing hidden button if any
          final existing = html.document.getElementById(
            'meta-pixel-submit-button',
          );
          existing?.remove();

          // Create a hidden HTML button that Meta can detect
          final hiddenButton = html.ButtonElement()
            ..id = 'meta-pixel-submit-button'
            ..style.display = 'none'
            ..setAttribute('data-fb-event-name', 'Purchase')
            ..setAttribute('data-fb-event-type', 'button')
            ..setAttribute('data-fb-content-name', 'Medicine Order')
            ..setAttribute('name', 'submit_order')
            ..setAttribute('aria-label', 'Submit Medicine Order')
            ..text = 'Submit Order';

          html.document.body?.append(hiddenButton);
        } catch (e) {
          debugPrint('Error creating Meta Pixel button: $e');
        }
      });
    }
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        onPressed: createOrderViewModel.loading
            ? null
            : () async {
                // Trigger hidden HTML button click for Meta detection (web only)
                if (kIsWeb) {
                  try {
                    final hiddenButton =
                        html.document.getElementById('meta-pixel-submit-button')
                            as html.ButtonElement?;
                    hiddenButton?.click();
                  } catch (e) {
                    debugPrint('Error triggering hidden button: $e');
                  }
                }
                ...
                if (instruction.isNotEmpty &&
                    !instructionRegex.hasMatch(instruction)) {
                  Utils.show(
                    "Special instructions can only contain letters and spaces",
                    context,
                  );
                  return;
                }
                ...
                if ((_selectedDuration == null || _selectedDuration!.isEmpty) &&
                  ...
                  Utils.show(
                    "Please select or enter medicine duration",
                    context,
                  );
                  return;
                }
                ...
                List<dynamic> filesToSend = kIsWeb
                    ? _webImages
                    : _selectedImages;
                ...
                try {
                  ...
                  // Track Meta Pixel Purchase event with URL parameters
                  _trackMetaPixelEvent(
                    'Purchase',
                    parameters: {
                      'content_name': 'Medicine Order',
                      'content_category': _selectedCategory ?? 'General',
                    },
                  );

                  // Also track in Google Analytics with URL parameters
                  UrlTrackingHelper.trackEventWithParams(
                    'purchase',
                    eventParams: {
                      'content_name': 'Medicine Order',
                      'content_category': _selectedCategory ?? 'General',
